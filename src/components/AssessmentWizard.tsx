...
export default function AssessmentWizard({ onComplete }: AssessmentWizardProps) {
  ...
  const getAquiferTypeByState = (state: string): 'Unconsolidated' | 'Semi-consolidated' | 'Consolidated' => {
    const stateUpper = state.toUpperCase();

    if (stateUpper.includes('TAMIL NADU')) {
      return 'Consolidated';
    } else if (stateUpper.includes('PUNJAB') || stateUpper.includes('HARYANA') ||
               stateUpper.includes('UTTAR PRADESH') || stateUpper.includes('BIHAR')) {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          const displayLocation = locationData.formatted || locationData.displayLocation || '';
          ...
          if (rainfallResponse.ok) {
            ...
            if (rainfallData.daily && rainfallData.daily.precipitation_sum) {
              ...
              updateField('annualRainfall', Math.round(totalRainfall).toString());
            }
          }
        } catch (error) {
      ...
      (error) => {
        ...
        alert('Unable to detect your location. Please ensure location permissions are enabled.');
        setIsDetectingLocation(false);
      }
    );
  };
  ...
  const handleSubmit = () => {
    const data: ProjectInput & {
      ...
      availableSpace: formData.availableSpace ? parseFloat(formData.availableSpace) : undefined,
    ...
    <div className="max-w-4xl mx-auto">
      ...
      >
        <div className="bg-gradient-to-r from-blue-600 to-teal-500 p-8 text-white">
          <h2 className="text-3xl font-bold mb-6">Assessment Wizard</h2>
                ...
                <div className="flex flex-col items-center">
                  <motion.div
                    animate={{
                      ...
                      backgroundColor: currentStep > step.num ? '#10B981' : currentStep === step.num ? '#fff' : 'rgba(255,255,255,0.3)'
                    }}
                    className={`w-12 h-12 rounded-full flex items-center justify-center font-bold transition-colors relative ${
              ...
              {currentStep === 2 && (
                <>
                  <div className="relative">
                    ...
                    <p className="text-sm text-gray-500 mt-2">Measure the total rooftop area available for collection</p>
                  </div>
                ...
                <>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => updateField('waterScarcityDays', e.target.value)}
                      placeholder=" "
                      min="1"
                      max="365"
                      className="peer w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none transition-all"
                    ...
                    <p className="text-sm text-gray-500 mt-2">Typical range: 30-200 days per year</p>
                  </div>
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      onChange={(e) => updateField('currentWaterSources', e.target.value)}
                      placeholder=" "
                      rows={3}
                ...
                <>
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => updateField('annualRainfall', e.target.value)}
                      placeholder=" "
                      min="100"
                      step="1"
                      className="peer w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none transition-all"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Aquifer Type *</label>
                    <div className="grid grid-cols-1 gap-3">
                      {[
                        { value: 'Unconsolidated', label: 'Unconsolidated', desc: 'Alluvial - Best for recharge', color: 'bg-green-100 border-green-300' },
                        { value: 'Semi-consolidated', label: 'Semi-consolidated', desc: 'Moderate suitability', color: 'bg-yellow-100 border-yellow-300' },
                        { value: 'Consolidated', label: 'Consolidated', desc: 'Hard rock - Limited recharge', color: 'bg-red-100 border-red-300' },
                      ].map((type) => (
                        <motion.button
                          key={type.value}
                          type="button"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) => updateField('depthWaterPremonsoon', e.target.value)}
                        placeholder=" "
                        min="0"
                        step="0.1"
                        className="peer w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none transition-all"
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) => updateField('depthWaterPostmonsoon', e.target.value)}
                        placeholder=" "
                        min="0"
                        step="0.1"
                        className="peer w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none transition-all"
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-gray-200">
            ...
            {currentStep < totalSteps ? (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="flex items-center px-8 py-3 bg-gradient-to-r from-blue-600 to-teal-500 text-white rounded-full font-medium shadow-lg hover:shadow-xl transition-shadow"
              >
                Next
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="flex items-center px-8 py-3 bg-gradient-to-r from-green-600 to-teal-500 text-white rounded-full font-medium shadow-lg hover:shadow-xl transition-shadow"
              >
